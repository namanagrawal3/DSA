...
public class Find_Building_Where_Alice_and_Bob_Can_Meet {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < q; i++) {
            ...
            if (minIdx == maxIdx)
                ...
                while (l <= r) {            // Binary Search on the index (Leftmost max index)
                    int mid = l + (r-l)/2;
    ...
    public static void buildSegmentTree(int[] heights, int i, int l, int r, int[] segTree) {
        if (l == r) {
            segTree[i] = l;
            return;
        }
    ...
    public static int rangeMax(int start, int end, int i, int l, int r, int[] heights, int[] segTree) {
        if (end < l || start > r)
            return -1;
        if (l >= start && r <= end)
            return segTree[i];
