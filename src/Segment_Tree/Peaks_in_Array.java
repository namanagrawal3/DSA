...
public class Peaks_in_Array {
    ...
    public static void buildSegmentTree(int[] prr, int i, int l, int r, int[] segTree) {
        if (l == r) {
            segTree[i] = prr[l];
            return;
        }
    ...
    public static void updateValue(int idx, int val, int i, int l, int r, int[] segTree) {
        if (l == r) {
            segTree[i] = val;
            return;
        }
    ...
    public static int countRange(int start, int end, int i, int l, int r, int[] segTree) {
        if (end < l || start > r)
            return 0;
        if (l >= start && r <= end)
            return segTree[i];
